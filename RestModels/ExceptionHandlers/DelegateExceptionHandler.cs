...
namespace RestModels.ExceptionHandlers {
	...
	public class DelegateExceptionHandler : IExceptionHandler {
		...
		/// </summary>
		private readonly Func<Exception, HttpContext, Task<bool?>> Handler;
		...
		public DelegateExceptionHandler(Func<Exception, HttpContext, Task<bool?>> handler) => this.Handler = handler;
		...
		/// </returns>
		public Task<bool?> HandleException(Exception exception, HttpContext context) => this.Handler(exception, context);
	}
}